...
namespace SkoroMario
{
    class Hra
    {
        ...
        private void InitializzeHlavniTimer()
        {
        ...
        private void hlavniAkce(object sender, EventArgs e)
        {
            ...
            if (hrac.skok > 0)
            {
                hrac.nastavPozici(platno, new Point(hrac.point.X, hrac.point.Y - hrac.rychlost));
                hrac.skok = hrac.skok - 1;
            }
            ...
            /* Hrac */
            Canvas.SetTop(hrac.obrazek, hrac.point.Y);
            Canvas.SetLeft(hrac.obrazek, hrac.point.X);
